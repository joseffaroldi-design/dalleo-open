      ...
      <div className="flex flex-col gap-6 rounded-3xl bg-forest p-7 shadow-md ring-1 ring-gold/40 sm:flex-row sm:items-center sm:justify-between sm:p-9">
        <div>
          <p className="flex items-center gap-3 text-sm font-bold uppercase tracking-[0.2em] text-gold">
            {final ? "2026 Final Results" : "Live Leaderboard"}
          ...
          <p className="mt-2 text-sm font-semibold text-cream/60 sm:text-base">
            ...
            · {final ? "Final result · 144 of 144 scores recorded" : `Thru ${leader.thru} holes`}
          </p>
        </div>
        <div className="flex shrink-0 flex-col gap-3 sm:flex-row sm:items-center">
          ...
          {!final && (
            <Link
              to="/score"
              data-testid="captain-signin-cta"
              className="inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-full border-2 border-gold/70 px-7 py-3 text-base font-extrabold text-gold-soft transition-all duration-200 hover:-translate-y-0.5 hover:bg-gold/10"
            >
              Captains — Sign In to Score
            </Link>
          )}
        </div>
      </div>
